...
def get_body(data_dict, database_id, media_status, media_type):
    ...
    if media_type == MediaType.MUSIC.value:
        ...
        # es_number = '  '.join(data_dict[MediaInfo.EPISODENUMBER.value])
        # single_lenght = ' , '.join(data_dict[MediaInfo.SINGLELENGHT.value])
        runtime = data_dict[MediaInfo.RUNTIME.value]
        # category = data_dict[MediaInfo.CATEGORIES.value]
        # log_detail.info(f" {data_dict[MediaInfo.RUNTIME.value]}")
        # 电视剧、电影、动画片（指的是电影）、动漫（剧集）、纪录片（电影和电视剧）
        # if category in ["电影", "动画片"]:
        #     time = runtime
        # elif category in ["电视剧", "动漫", "纪录片"]:
        #     if not es_number:
        #         if single_lenght:
        #             time = f'单集片长:{single_lenght}'
        #         else:
        #             time = None
        #     else:
        #         if single_lenght:
        #             time = f'集数:{es_number}集, 单集片长:{single_lenght}'
        #         else:
        #             time = f'集数:{es_number}集'
        # else:
        #     time = None  # 如果不是以上任何一种类别，time为空
        body = {
            "parent": {
                "type": "database_id",
                "database_id": f"{database_id}"
        ...
        if data_dict[MediaInfo.RATING_F.value]:
            ...
            body["properties"]["评分"] = tmp_dict
        # 片长
        # if data_dict[MediaInfo.RUNTIME.value]:
        #     log_detail.info(f" {data_dict[MediaInfo.RUNTIME.value]}")
            # rating_f = float(data_dict[MediaInfo.RATING_F.value])
            # tmp_dict = get_non_null_params_body(property_type=DatabaseProperty.NUMBER.value,
            #                                     property_params=rating_f)
            # body["properties"]["片长"] = tmp_dict
        # 评分人数
        if data_dict[MediaInfo.ASSESS.value]:
            tmp_dict = get_non_null_params_body(property_type=DatabaseProperty.NUMBER.value,
                                                property_params=data_dict[MediaInfo.ASSESS.value])
