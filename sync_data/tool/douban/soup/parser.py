...
class ParserHtmlText:
    ...
    def __get_movie_dict(self):
        ...
        if '上映时间:' in infos or '上映日期:' in infos:
            ...
            movie_categories = "未知"
        # 根据电影类别设置 runtime
        if movie_categories in ["电影", "动画片"]:
            runtime_list = get_multiple_infos_list(infos, "片长:", 1)
            runtime = runtime_list[0] if runtime_list else None
        elif movie_categories in ["电视剧", "动漫", "纪录片", "未知"]:
            if '集数:' in infos:
                episodeNumberList = get_multiple_infos_list(infos, "集数:", 1)
                episodeNumber = episodeNumberList[0] if episodeNumberList else None
                if '单集片长:' in infos:
                    singleLenghtList = get_multiple_infos_list(infos, "单集片长:", 1)
                    singleLenght = singleLenghtList[0] if singleLenghtList else None
                    if singleLenght != "":  # 明确检查空字符串
                        # 你的代码赋值进入这个分支
                        runtime = f'集数:{episodeNumber}集, 单集片长:{singleLenght}'
                else:
                    if episodeNumber != "":
                        runtime = f'集数:{episodeNumber}集'
            elif '单集片长:' in infos:
                log_detail.info(f'lililililili')
                singleLenghtList = get_multiple_infos_list(infos, "单集片长:", 1)
                singleLenght = singleLenghtList[0] if singleLenghtList else None
                if singleLenght != "":
                    runtime = f'单集片长:{singleLenght}'
            else:
                runtime = ' '
        ...
        movie_dict[MediaInfo.RUNTIME.value] = runtime
        # movie_dict[MediaInfo.SINGLELENGHT.value] = singleLenght
        # movie_dict[MediaInfo.EPISODENUMBER.value] = episodeNumber
    ...
    def get_my_movie_rating(self):
        rating_input = self.soup.find(id='n_rating')
        if rating_input and 'value' in rating_input.attrs:
            my_rating = rating_input['value']
            print(f"找到评分: {my_rating}")
        else:
            print("没有找到#n_rating元素或其value属性")
